...
├── [채널슬러그]/
│   ├── _context.md         # ★ 병원별 리포트 컨텍스트 SSOT (매달 먼저 읽기)
│   └── YYYY-MM.html        # 자체완결 월간 리포트 (CSS 내장)
└── README.md
```

## _context.md — 병원별 SSOT (핵심)

각 채널 폴더의 `_context.md` = 그 병원 리포트의 **반복 컨텍스트**. 매달 처음부터 분석하지 않기 위해, 검증된 결론을 누적한다.
담는 것: 채널 정체성 · **타겟 환자 정의** · 주력 질환 · 영상 각도(효과순) · KPI · 분류 키워드(스크립트용) · 월별 baseline(전월대비용) · **내원 추적 로그** · 리포트 히스토리.

**매달 작업 시작 = `_context.md` 먼저 읽기.** 새 데이터로 baseline·내원로그·히스토리를 갱신하고, 그 위에서 리포트 생성.
